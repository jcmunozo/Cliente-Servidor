...
public class GestorClientes extends AModel {
    ...
    public int getTotalHombres() throws ClassNotFoundException, SQLException {
        conector.conectarse();
        conector.crearConsulta("SELECT * FROM clientes WHERE sexo = 'Masculino'");

        ArrayList<Cliente> clientesHombres = new ArrayList();

        while (conector.getResultado().next()) {
            Cliente cli = new Cliente(conector.getResultado().getString("id"), conector.getResultado().getString("nombres"), conector.getResultado().getString("apellidos"), conector.getResultado().getString("direccion"), conector.getResultado().getString("celular"), conector.getResultado().getString("email"), conector.getResultado().getString("sexo"));
            clientesHombres.add(cli);
        }
        conector.desconectarse();
        return clientesHombres.size();
        
    }
    
    public int getTotalHombresV2() throws ClassNotFoundException, SQLException {
        conector.conectarse();
        conector.crearConsulta("SELECT COUNT(*) as total FROM clientes WHERE sexo = 'Masculino'");

        int clientesHombres = 0 ;
        if (conector.getResultado().next()){
            clientesHombres = conector.getResultado().getInt("total");
        }
        return clientesHombres;
    }
    public int getTotalMujeresV2() throws ClassNotFoundException, SQLException {
        conector.conectarse();
        conector.crearConsulta("SELECT COUNT(*) as total FROM clientes WHERE sexo = 'Femenino'");

        int mujeres = 0 ;
        if (conector.getResultado().next()){
            mujeres = conector.getResultado().getInt("total");
        }
        return mujeres;
    }
    
    public int getTotalMujeres() throws SQLException, ClassNotFoundException{
        conector.conectarse();
        conector.crearConsulta("SELECT * FROM clientes WHERE sexo = 'Femenino'");

        ArrayList<Cliente> clientesMujeres = new ArrayList();

        while (conector.getResultado().next()) {
            Cliente cli = new Cliente(conector.getResultado().getString("id"), conector.getResultado().getString("nombres"), conector.getResultado().getString("apellidos"), conector.getResultado().getString("direccion"), conector.getResultado().getString("celular"), conector.getResultado().getString("email"), conector.getResultado().getString("sexo"));
            clientesMujeres.add(cli);
        }
        conector.desconectarse();
        return clientesMujeres.size();
    }     
}
